...
# municipios_general <- indicadores %>% dplyr::select(depto_mun) %>% 
#  distinct() %>% rename(indicadores_depto_mun=depto_mun)
...
indicadores_BD<- merge(indicadores, Economia_depto,
                       by.x= "departamento_entidad", by.y= "departamento", 
                       all.x=TRUE)
...
indicadores_BD<- merge(indicadores_BD, Economia_municipio, 
                       by.x="depto_mun.x", by.y="depto_mun",
...
# guardarlo como csv con encoding utf-8
write.csv(indicadores_BD, 
          file = "indicadores_BD.csv",
          row.names = TRUE,
          encoding="UTF-8")
