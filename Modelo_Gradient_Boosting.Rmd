...
```{r}
indicadores_gen_rec <- recipe(not_rejected_sin_mdm_supers, data = indicadores_gen) %>% 
...
```{r}
XGB_Curva_ROC_sin_mdm_supers <- final_res %>% 
  collect_predictions() %>%
...
```{r}
ggsave("XGB_Curva_ROC_sin_mdm_superss.png", 
       XGB_Curva_ROC_sin_mdm_supers)
```
...
XGB_importancia_variables_sin_mdm_supers <- final_xgb %>%
  fit(data = vb_train) %>%
  pull_workflow_fit() %>%
  vip(geom = "col",
      num_features = 20L) + 
...
XGB_importancia_variables_sin_mdm_supers
```
Guardar imagen

```{r}
ggsave("XGB_importancia_variables_sin_mdm_supers.png",
       XGB_importancia_variables_sin_mdm_supers)
```
...
```{r}
# Total
XGB_Importancia_subcategorias_total_sin_mdm_supers <- separate(data = importantes_xgb,
         col = variable,
         into = "inicio",
         sep = "_") %>% 
  group_by(inicio) %>% 
...
#Promedio
XGB_Importancia_subcategorias_media_sin_mdm_supers <- separate(data = importantes_xgb,
         col = variable,
         into = "inicio",
         sep = "_") %>% 
  group_by(inicio) %>% 
...
XGB_Importancia_subcategorias_total_sin_mdm_supers
XGB_Importancia_subcategorias_media_sin_mdm_supers
```
...
```{r}
# Total
ggsave("XGB_Importancia_subcategorias_total_sin_mdm_supers.png",
       XGB_Importancia_subcategorias_total_sin_mdm_supers)

# Media
ggsave("XGB_Importancia_subcategorias_media_sin_mdm_supers.png",
       XGB_Importancia_subcategorias_media_sin_mdm_supers)
```
...
```{r}
# Suma total
XGB_Importancia_niveles_total_sin_mdm_supers <- importantes_xgb %>%
...
# Suma media
XGB_Importancia_niveles_media_sin_mdm_supers <- importantes_xgb %>%
  filter(fin == "hos" | fin == "mun" | fin == "dto") %>% 
  group_by(fin) %>% 
...
XGB_Importancia_niveles_total_sin_mdm_supers
XGB_Importancia_niveles_media_sin_mdm_supers
```
...
```{r}
# Total
ggsave("XGB_Importancia_niveles_total_sin_mdm_supers.png",
       XGB_Importancia_niveles_total_sin_mdm_supers)

# Media
ggsave("XGB_Importancia_niveles_media_sin_mdm_supers.png",
       XGB_Importancia_niveles_media_sin_mdm_supers)
```
...
XGB_Importancia_años_total_sin_mdm_supers <- importantes_xgb %>% 
...
XGB_Importancia_años_media_sin_mdm_supers <- importantes_xgb %>% 
  mutate(variable = gsub("_1_", "un", variable),
         variable = gsub("IC4K", "ICFK", variable),
         variable = gsub("_5_", "_cinco_", variable),
         variable = gsub("ind2", "ind_dos", variable),
...
XGB_Importancia_años_total_sin_mdm_supers
XGB_Importancia_años_media_sin_mdm_supers
```
...
```{r}
#Total
ggsave("XGB_Importancia_años_total_sin_mdm_supers.png",
       XGB_Importancia_años_total_sin_mdm_supers)

#Media
ggsave("XGB_Importancia_años_media_sin_mdm_supers.png",
       XGB_Importancia_años_media_sin_mdm_supers)
```
...
```{r}
indicadores_gen_varfin_sin_mdm_supers <- indicadores_gen %>% 
...
```{r}
pca_xgboost_rec_sin_mdm_supers <- recipe(riesgo ~ ., data = indicadores_gen_varfin_sin_mdm_supers) %>%
...
pca_xgboost_prep_sin_mdm_supers <- prep(pca_xgboost_rec_sin_mdm_supers)

pca_xgboost_prep_sin_mdm_supers
```
...
```{r}
XGB_PCA_hospitales_sin_mdm_supers <- juice(pca_xgboost_prep_sin_mdm_supers) %>%
...
XGB_PCA_hospitales_sin_mdm_supers
```
...
```{r}
#Sin leyenda
ggsave("XGB_PCA_hospitales_sin_mdm_supers_sin_leyenda.png",
       XGB_PCA_hospitales_sin_mdm_supers)

#Con leyenda
ggsave(
  "XGB_PCA_hospitales_sin_mdm_supers_con_leyenda.png",
  juice(pca_xgboost_prep_sin_mdm_supers) %>%
  ggplot(aes(PC1, PC2, label = departamento_entidad)) +
...
```{r}
umap_xgboost_rec_sin_mdm_supers <- recipe(riesgo ~ ., data = indicadores_gen_varfin_sin_mdm_supers) %>%
  update_role(nombre_entidad, departamento_entidad, new_role = "id") %>%
  step_dummy(all_nominal(), - c(nombre_entidad, departamento_entidad), one_hot = TRUE) %>% 
  step_normalize(all_predictors()) %>%
  step_umap(all_predictors())

umap_xgboost_prep_sin_mdm_supers <- prep(umap_xgboost_rec_sin_mdm_supers)

umap_xgboost_prep_sin_mdm_supers
```
...
```{r}
XGB_UMAP_hospitales_sin_mdm_supers <- juice(umap_xgboost_prep_sin_mdm_supers) %>%
...
XGB_UMAP_hospitales_sin_mdm_supers
```
...
```{r}
#sin leyenda
ggsave("XGB_UMAP_sin_mdm_supers_sin_leyenda.png",
       XGB_UMAP_hospitales_sin_mdm_supers)

#con leyenda
ggsave(
  "XGB_UMAP_sin_mdm_supers_con_leyenda.png",
juice(umap_xgboost_prep_sin_mdm_supers) %>%
  ggplot(aes(umap_1, umap_2, label = departamento_entidad)) +
  geom_point(aes(color = departamento_entidad), alpha = 0.7, size = 2) +
  geom_text(check_overlap = TRUE, hjust = "inward", family = "IBMPlexSans") +
  labs(color = NULL,
