...
```{r}
XGB_Curva_ROC_sin_boruta <- final_res %>% 
  collect_predictions() %>%
...
```{r}
ggsave("XGB_Curva_ROC_sin_boruta.png", 
       XGB_Curva_ROC_sin_boruta)
```
...
XGB_importancia_variables_sin_boruta <- final_xgb %>%
  fit(data = vb_train) %>%
  pull_workflow_fit() %>%
  vip(geom = "col",
      num_features = 20L) + 
...
XGB_importancia_variables_sin_boruta
```
Guardar imagen

```{r}
ggsave("XGB_importancia_variables_sin_boruta.png",
       XGB_importancia_variables_sin_boruta)
```
...
```{r}
# Total
XGB_Importancia_subcategorias_total_sin_boruta <- separate(data = importantes_xgb,
         col = variable,
         into = "inicio",
         sep = "_") %>% 
  group_by(inicio) %>% 
...
#Promedio
XGB_Importancia_subcategorias_media_sin_boruta <- separate(data = importantes_xgb,
         col = variable,
         into = "inicio",
         sep = "_") %>% 
  group_by(inicio) %>% 
...
XGB_Importancia_subcategorias_total_sin_boruta
XGB_Importancia_subcategorias_media_sin_boruta
```
...
```{r}
# Total
ggsave("XGB_Importancia_subcategorias_total_sin_boruta.png",
       XGB_Importancia_subcategorias_total_sin_boruta)

# Media
ggsave("XGB_Importancia_subcategorias_media_sin_boruta.png",
       XGB_Importancia_subcategorias_media_sin_boruta)
```
...
XGB_Importancia_años_total_sin_boruta <- importantes_xgb %>% 
...
XGB_Importancia_años_media_sin_boruta <- importantes_xgb %>% 
  mutate(variable = gsub("_1_", "un", variable),
         variable = gsub("IC4K", "ICFK", variable),
         variable = gsub("_5_", "_cinco_", variable),
         variable = gsub("ind2", "ind_dos", variable),
...
XGB_Importancia_años_total_sin_boruta
XGB_Importancia_años_media_sin_boruta
```
...
```{r}
#Total
ggsave("XGB_Importancia_años_total_sin_boruta.png",
       XGB_Importancia_años_total_sin_boruta)

#Media
ggsave("XGB_Importancia_años_media_sin_boruta.png",
       XGB_Importancia_años_media_sin_boruta)
```
...
```{r}
indicadores_gen_varfin_sin_boruta <- indicadores_gen %>% 
...
```{r}
pca_xgboost_rec_sin_boruta <- recipe(riesgo ~ ., data = indicadores_gen_varfin_sin_boruta) %>%
...
pca_xgboost_prep_sin_boruta <- prep(pca_xgboost_rec_sin_boruta)

pca_xgboost_prep_sin_boruta
```
...
```{r}
XGB_PCA_hospitales_sin_boruta <- juice(pca_xgboost_prep_sin_boruta) %>%
...
```{r}
#Sin leyenda
ggsave("XGB_PCA_hospitales_sin_boruta_sin_leyenda.png",
       XGB_PCA_hospitales_sin_boruta)

#Con leyenda
ggsave(
  "XGB_PCA_hospitales_sin_boruta_con_leyenda.png",
  juice(pca_xgboost_prep_sin_boruta) %>%
  ggplot(aes(PC1, PC2, label = departamento_entidad)) +
...
```{r}
umap_xgboost_rec_sin_boruta <- recipe(riesgo ~ ., data = indicadores_gen_varfin_sin_boruta) %>%
  update_role(nombre_entidad, departamento_entidad, new_role = "id") %>%
  step_dummy(all_nominal(), - c(nombre_entidad, departamento_entidad), one_hot = TRUE) %>% 
  step_normalize(all_predictors()) %>%
  step_umap(all_predictors())

umap_xgboost_prep_sin_boruta <- prep(umap_xgboost_rec_sin_boruta)

umap_xgboost_prep_sin_boruta
```
...
```{r}
XGB_UMAP_hospitales_sin_boruta <- juice(umap_xgboost_prep_sin_boruta) %>%
...
XGB_UMAP_hospitales_sin_boruta
```
...
```{r}
#sin leyenda
ggsave("XGB_UMAP_sin_boruta_sin_leyenda.png",
       XGB_UMAP_hospitales_sin_boruta)

#con leyenda
ggsave(
  "XGB_UMAP_sin_boruta_con_leyenda.png",
juice(umap_xgboost_prep_sin_boruta) %>%
  ggplot(aes(umap_1, umap_2, label = departamento_entidad)) +
  geom_point(aes(color = departamento_entidad), alpha = 0.7, size = 2) +
  geom_text(check_overlap = TRUE, hjust = "inward", family = "IBMPlexSans") +
  labs(color = NULL,
