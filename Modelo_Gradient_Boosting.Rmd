...
indicadores_gen$indba_perc_direc_val <- NULL #porque se supone que no tenemos info de contr. 
indicadores_gen$indba_perc_cerrada_num <- NULL
indicadores_gen$indba_perc_cerrada_val <- NULL

#para volver a correr el modelo porque con esta columna genera error 
indicadores_gen$nombre_entidad <- NULL

# eliminar supersalud que no sé manejar jeje
indicadores_gen$supers_ind2_efectividad_en_auditoria <- NULL

#eliminar otros indicadores de supersalud que no yudaron
indicadores_gen$supers_perc_pacientes_hipertension <- NULL
indicadores_gen$supers_oportun_atencion_consulta_odonto_general <- NULL
indicadores_gen$supers_oportun_asign_citas_consulta_general <- NULL
indicadores_gen$supers_tasa_satis_global <- NULL
...
#indicadores_gen$riesgo <- NULL
#Ccalcular los cuartiles
#cartiles <-quantile(indicadores_gen$perc_contr_directa_num)
...
#indicadores_gen %>% dplyr::select(perc_contr_directa_num, quartil)

# hacer histograma con la distribucion de los cuartiles
ggplot(indicadores_gen, aes(indba_perc_cdirec_num)) + 
...
```{r}
indicadores_gen$indba_perc_cdirec_num <- NULL

#indicadores_gen$supers_ind10_oportunidad_entrega_reporte_info[is.na(indicadores_gen$supers_ind10_oportunidad_entrega_reporte_info)] <- "NO REGISTRA"

#head(indicadores_gen[,182:186], 10)
...
```{r}
indicadores_gen_rec <- recipe(riesgo ~ .,
                               data = indicadores_gen) %>% 
