...
```{r}
XGB_Curva_ROC_todas_las_variables <- final_res %>% 
  collect_predictions() %>%
...
XGB_Curva_ROC_todas_las_variables
```
...
```{r}
ggsave("XGB_Curva_ROC_todas_las_variables.png", 
       XGB_Curva_ROC_todas_las_variables)
```
...
XGB_importancia_variables_todas_las_variables <- final_xgb %>%
  fit(data = vb_train) %>%
  pull_workflow_fit() %>%
  vip(geom = "col",
      num_features = 20L) + 
...
XGB_importancia_variables_todas_las_variables
```
Guardar imagen

```{r}
ggsave("XGB_importancia_variables_todas_las_variables.png",
       XGB_importancia_variables_todas_las_variables)
```
...
```{r}
# Total
XGB_Importancia_subcategorias_total_todas_las_variables <- separate(data = importantes_xgb,
         col = variable,
         into = "inicio",
         sep = "_") %>% 
  group_by(inicio) %>% 
...
#Promedio
XGB_Importancia_subcategorias_media_todas_las_variables <- separate(data = importantes_xgb,
         col = variable,
         into = "inicio",
         sep = "_") %>% 
  group_by(inicio) %>% 
...
XGB_Importancia_subcategorias_total_todas_las_variables
XGB_Importancia_subcategorias_media_todas_las_variables
```
...
```{r}
# Total
ggsave("XGB_Importancia_subcategorias_total_todas_las_variables.png",
       XGB_Importancia_subcategorias_total_todas_las_variables)

# Media
ggsave("XGB_Importancia_subcategorias_media_todas_las_variables.png",
       XGB_Importancia_subcategorias_media_todas_las_variables)
```
...
```{r}
# Suma total
XGB_Importancia_niveles_total_todas_las_variables <- importantes_xgb %>%
...
# Suma media
XGB_Importancia_niveles_media_todas_las_variables <- importantes_xgb %>%
  filter(fin == "hos" | fin == "mun" | fin == "dto") %>% 
  group_by(fin) %>% 
...
XGB_Importancia_niveles_total_todas_las_variables
XGB_Importancia_niveles_media_todas_las_variables
```
...
```{r}
# Total
ggsave("XGB_Importancia_niveles_total_todas_las_variables.png",
       XGB_Importancia_niveles_total_todas_las_variables)

# Media
ggsave("XGB_Importancia_niveles_media_todas_las_variables.png",
       XGB_Importancia_niveles_media_todas_las_variables)
```
...
XGB_Importancia_años_total_todas_las_varibles <- importantes_xgb %>% 
...
XGB_Importancia_años_media_todas_las_varibles <- importantes_xgb %>% 
  mutate(variable = gsub("_1_", "un", variable),
         variable = gsub("IC4K", "ICFK", variable),
         variable = gsub("_5_", "_cinco_", variable),
         variable = gsub("ind2", "ind_dos", variable),
...
XGB_Importancia_años_total_todas_las_varibles
XGB_Importancia_años_media_todas_las_varibles
```
...
```{r}
#Total
ggsave("XGB_Importancia_años_total_todas_las_varibles.png",
       XGB_Importancia_años_total_todas_las_varibles)

#Media
ggsave("XGB_Importancia_años_media_todas_las_varibles.png",
       XGB_Importancia_años_media_todas_las_varibles)
```
...
```{r}
indicadores_gen_varfin <- indicadores_gen %>% 
...
```{r}
pca_xgboost_rec <- recipe(riesgo ~ ., data = indicadores_gen_varfin) %>%
...
pca_xgboost_prep <- prep(pca_xgboost_rec)
...
```{r}
XGB_PCA_hospitales_todas_las_variables <- juice(pca_xgboost_prep) %>%
...
XGB_PCA_hospitales_todas_las_variables
```
...
```{r}
#Sin leyenda
ggsave("XGB_PCA_hospitales_todas_las_variables_sin_leyenda.png",
       XGB_PCA_hospitales_todas_las_variables)

#Con leyenda
ggsave(
  "XGB_PCA_hospitales_todas_las_variables_con_leyenda.png",
  juice(pca_xgboost_prep) %>%
  ggplot(aes(PC1, PC2, label = departamento_entidad)) +
...
```{r}
umap_xgboost_rec <- recipe(riesgo ~ ., data = indicadores_gen_varfin) %>%
  update_role(nombre_entidad, departamento_entidad, new_role = "id") %>%
  step_dummy(all_nominal(), - c(nombre_entidad, departamento_entidad), one_hot = TRUE) %>% 
  step_normalize(all_predictors()) %>%
  step_umap(all_predictors())

umap_xgboost_prep <- prep(umap_xgboost_rec)
...
```{r}
XGB_UMAP_hospitales_todas_las_variables <- juice(umap_xgboost_prep) %>%
...
XGB_UMAP_hospitales_todas_las_variables
```
...
```{r}
#sin leyenda
ggsave("XGB_UMAP_todas_las_variables_sin_leyenda.png",
       XGB_UMAP_hospitales_todas_las_variables)

#con leyenda
ggsave(
  "XGB_UMAP_todas_las_variables_con_leyenda.png",
juice(umap_xgboost_prep) %>%
  ggplot(aes(umap_1, umap_2, label = departamento_entidad)) +
  geom_point(aes(color = departamento_entidad), alpha = 0.7, size = 2) +
  geom_text(check_overlap = TRUE, hjust = "inward", family = "IBMPlexSans") +
  labs(color = NULL,
