...
```{r}
indicadores <- read_delim("Indicadores_Final_Anno.csv", ";")
head(indicadores)
#apply(is.na(indicadores), 2, cantyprop_nas)
```
...
Reemplazar la columna con el nombre del departamento-municipio
...
Agregar el indicativo _hos a los indicadores

```{r}
indicadores <- indicadores %>% 
  pivot_longer(cols = !c(nombre_entidad, nit_entidad, orden_entidad, depto_mun, departamento_entidad, municipio_entidad), names_to = "indicador", values_to = "valor") %>% 
  mutate(indicador = paste(indicador, "hos", sep = "_")) %>% 
...
economia_depto <- economia_depto %>% 
  mutate(# Ajsutes de escritura
         indicador_annio = tolower(indicador_annio),
         indicador_annio = chartr('ÁÉÍÓÚáéíóú ','AEIOUaeiou_', indicador_annio),
         indicador_annio = gsub("%", "perc", indicador_annio),
         indicador_annio = gsub("porcentaje", "perc", indicador_annio),
         indicador_annio = gsub(",", "", indicador_annio),
         indicador_annio = gsub("_de_", "_", indicador_annio),
         indicador_annio = gsub("_y_", "_", indicador_annio),
         indicador_annio = gsub("_al_", "_", indicador_annio),
         indicador_annio = gsub("_del_", "_", indicador_annio),
         #Añadir el indicativo de economia "inec"
         indicador_annio = str_c("inec", indicador_annio, "dto", sep = "_"))
```
...
economia_municipio <- economia_municipio %>% 
  mutate(# Ajsutes de escritura
         indicador_annio = tolower(indicador_annio),
         indicador_annio = gsub(' - ','_', indicador_annio),
         indicador_annio = chartr('ÁÉÍÓÚáéíóú ','AEIOUaeiou_', indicador_annio),
         indicador_annio = gsub("porcentaje", "perc", indicador_annio),
         indicador_annio = gsub(",", "", indicador_annio),
         indicador_annio = gsub("_en_", "_", indicador_annio),
         indicador_annio = gsub("_el_", "_", indicador_annio),
         indicador_annio = gsub("_del_", "_", indicador_annio),
         indicador_annio = gsub("valor_agregado_", "vagregado_", indicador_annio),
         indicador_annio = gsub("vagregado_por_actividades_economicas_", "vagregado_", indicador_annio),
         #Añadir el indicativo de economia "inec"
         indicador_annio = str_c("inec", indicador_annio, "mun", sep = "_"))
```
...
Ver los Nas y su propoción por cada columna

```{r}
apply(is.na(indicadores_bd_td), 2, cantyprop_nas)
```

Reemplazar los na en el cod_departamento bogotá

```{r}
indicadores_bd_td <- as.data.table(indicadores_bd_td)
indicadores_bd_td[departamento_entidad == "Bogotá", cod_departamento := "11001"]
```
...
#indicadores_bd_td <- indicadores_bd_td[, 1:118]
indicadores_bd_td <- merge(indicadores_bd_td,
...
indicadores_bd_td$cod_entidad <- as.character(indicadores_bd_td$cod_entidad)
indicadores_bd_td <- merge(indicadores_bd_td,
                           educacion_municipio,
                           by.x = "cod_entidad",
                           by.y = "codigo_entidad", 
...
mdm$cod_entidad <- as.character(mdm$cod_entidad)
...
Renombrar las variables de la base entidades que se van a utilizar

```{r}
entidades <- entidades %>% 
  rename("indba_ptr_contratos_minima_cuantia_hos" = indba_contratos_minima_cuantia_hos,
         "indba_ptr_prop_contr_minima_cuantia_hos" = indba_prop_contr_minima_cuantia_hos,
         "indba_ptr_valor_contratos_minima_cuantia_hos" = indba_valor_contratos_minima_cuantia_hos,
         "indba_ptr_prop_val_minima_cuantia_hos" = indba_prop_val_minima_cuantia_hos,
         "indba_ptr_contratos_minima_cuantia_dto" = indba_contratos_minima_cuantia_dto,
         "indba_ptr_prop_contr_minima_cuantia_dto" = indba_prop_contr_minima_cuantia_dto,
         "indba_ptr_valor_contratos_minima_cuantia_dto" = indba_valor_contratos_minima_cuantia_dto,
         "indba_ptr_prop_val_minima_cuantia_dto" = indba_prop_val_minima_cuantia_dto,
         "indba_ptr_contratos_adiciones_hos" = indba_contratos_adiciones_hos)
```
...
deptos_finyr <- SECOP_I %>%
  ...
  count(departamento_entidad, anno_firma) %>%
  rename("indba_ptr_contr_finyr" = "n") %>% 
  pivot_longer(cols = indba_ptr_contr_finyr,
               names_to = "indicador",
               values_to = "valor") %>% 
  mutate(indicador = paste(indicador, anno_firma, "dto", sep = "_")) %>% 
  dplyr::select(-anno_firma) %>% 
...
#ver los nas
apply(is.na(deptos_finyr), 2, cantyprop_nas)
...
```{r}
hospitales_finyr <- SECOP_I %>%
    filter(fecha_firma_contrato >= as.Date("2015-12-01") & fecha_firma_contrato <= as.Date("2015-12-31") |
         fecha_firma_contrato >= as.Date("2016-12-01") & fecha_firma_contrato <= as.Date("2016-12-31") |
         fecha_firma_contrato >= as.Date("2017-12-01") & fecha_firma_contrato <= as.Date("2017-12-31") | 
         fecha_firma_contrato >= as.Date("2018-12-01") & fecha_firma_contrato <= as.Date("2018-12-31") | 
         fecha_firma_contrato >= as.Date("2019-12-01") & fecha_firma_contrato <= as.Date("2019-12-31")) %>%
  count(nombre_entidad, departamento_entidad, anno_firma) %>%
  rename("indba_ptr_contr_finyr" = "n") %>% 
  pivot_longer(cols = indba_ptr_contr_finyr,
               names_to = "indicador",
               values_to = "valor") %>% 
  mutate(indicador = paste(indicador, anno_firma, "hos", sep = "_")) %>% 
  dplyr::select(-anno_firma) %>% 
  pivot_wider(names_from = indicador,
              values_from = valor) %>% 
  dplyr::select(-indba_ptr_contr_finyr_2015_hos) #se elimina 2015 porque tiene NAs en mas del 40% de los datos

# Cambiar nos NA por ceros
hospitales_finyr <- data.frame(sapply(hospitales_finyr, cambio_na))

#ver los nas
apply(is.na(hospitales_finyr), 2, cantyprop_nas)
...
```{r}
#indicadores_bd_td <- indicadores_bd_td[, 1:312]
# entidades con indicadores_bt_td
indicadores_bd_td <- merge(indicadores_bd_td,
                           entidades %>% dplyr::select("indba_nombre_entidad_hos",
                                                       "indba_ptr_contratos_minima_cuantia_hos",
                                                       "indba_ptr_prop_contr_minima_cuantia_hos",
                                                       "indba_ptr_valor_contratos_minima_cuantia_hos",
                                                       "indba_ptr_prop_val_minima_cuantia_hos",
                                                       "indba_ptr_contratos_minima_cuantia_dto",
                                                       "indba_ptr_prop_contr_minima_cuantia_dto",
                                                       "indba_ptr_valor_contratos_minima_cuantia_dto",
                                                       "indba_ptr_prop_val_minima_cuantia_dto",
                                                       "indba_ptr_contratos_adiciones_hos"),
                           by.x = "nombre_entidad",
...
```{r}
#indicadores_bd_td <- indicadores_bd_td[, 1:321]
indicadores_bd_td <- merge(indicadores_bd_td,
                           hospitales_finyr,
                           by = "nombre_entidad",
                           all.x = TRUE)
...
indicadores_bd_td <- indicadores_bd_td %>% 
  mutate(indba_ptr_prop_contr_finyr_2016_hos = (indba_ptr_contr_finyr_2016_hos / indba_ptr_total_cont_2016_hos )*100,
         indba_ptr_prop_contr_finyr_2017_hos = (indba_ptr_contr_finyr_2017_hos / indba_ptr_total_cont_2017_hos )*100,
         indba_ptr_prop_contr_finyr_2018_hos = (indba_ptr_contr_finyr_2018_hos / indba_ptr_total_cont_2018_hos )*100,
         indba_ptr_prop_contr_finyr_2019_hos = (indba_ptr_contr_finyr_2019_hos / indba_ptr_total_cont_2019_hos )*100)
```
...
indicadores_bd_td <- merge(x = indicadores_bd_td,
  ...
  rename("supers_ind2_efectividad_en_auditoria_hos" =
           "Indicador2: Efectividad en la auditoría para el mejoramiento continuo de la calidad",
         "supers_ind10_oportunidad_entrega_reporte_info_hos" =
           "Indicador10: Oportunidad en la entrega del reporte de información en Cumplimiento de la Circular Única o norma que la sustituya.")
```

### 3.4.4. Ajuste de las variables tipo factor
...
indicadores_bd_td$supers_ind10_oportunidad_entrega_reporte_info_hos[is.na(indicadores_bd_td$supers_ind10_oportunidad_entrega_reporte_info_hos)] <- "0"
...
Eliminar variables con proporción de Nas superior al 40%

```{r}
indicadores_bd_td <- indicadores_bd_td %>% 
  dplyr::select( -c( #Financiero
    "inf_cantidad_proyectos_2015_mun",
    "inf_valor_proyectos_2018_mun",
    # Salud
    "ins_razon_de_mortalidad_materna_a_42_dias_2014_mun",
...
## 3.5. Info contratistas

Repitencia de los contratistas

```{r}
repitencia_contratista <- SECOP_I %>% count(nombre_entidad, id_contratista) %>% 
  rename(indba_ptr_repitencia_contratista_hos = n) %>% 
  dplyr::select(-id_contratista) %>% 
  group_by(nombre_entidad) %>% 
  top_n(n =1, wt = indba_ptr_repitencia_contratista_hos) %>% 
  unique()

apply(is.na(repitencia_contratista), 2, cantyprop_nas)

head(repitencia_contratista %>% arrange(desc(indba_ptr_repitencia_contratista_hos)))
```

Unir la repitencia contratista con la cantidad de contratistas por cada hospital por año

```{r}
ptr_contratistas <- merge(repitencia_contratista,
                          SECOP_I %>% 
  group_by(nombre_entidad, anno_firma) %>% 
  summarise(indba_ptr_cantidad_contratistas_hos = n_distinct(id_contratista)) %>% 
  mutate(indicador_annio = str_c("indba_ptr_cantidad_contratistas", anno_firma, "hos", sep = "_")) %>% 
  dplyr::select(nombre_entidad, indicador_annio, indba_ptr_cantidad_contratistas_hos) %>% 
  pivot_wider(names_from = indicador_annio, values_from = indba_ptr_cantidad_contratistas_hos),
                          by = "nombre_entidad"
                          ) %>% 
  dplyr::select(-indba_ptr_cantidad_contratistas_2014_hos)

head(ptr_contratistas)

# ver las cantidades y proporciones de Nas
apply(is.na(ptr_contratistas), 2, cantyprop_nas)
```

Unir ptr_contratistas con la base de datos
...
Calcular en a cada contratista aproximadamente cuántos contratos le corresponden y el valor medio de los contratos por contratista

```{r}
indicadores_bd_td <- indicadores_bd_td %>% 
  mutate( #Contratos por contratista
    indba_idr_contratos_por_contratista_2015_hos = indba_ptr_total_cont_2015_hos / indba_ptr_cantidad_contratistas_2015_hos,
    indba_idr_contratos_por_contratista_2016_hos = indba_ptr_total_cont_2016_hos / indba_ptr_cantidad_contratistas_2016_hos,
    indba_idr_contratos_por_contratista_2017_hos = indba_ptr_total_cont_2017_hos / indba_ptr_cantidad_contratistas_2017_hos,
    indba_idr_contratos_por_contratista_2018_hos = indba_ptr_total_cont_2018_hos / indba_ptr_cantidad_contratistas_2018_hos,
    indba_idr_contratos_por_contratista_2019_hos = indba_ptr_total_cont_2019_hos / indba_ptr_cantidad_contratistas_2019_hos,
         #Valor de los contratos por contratista
    indba_idr_valor_contratos_por_contratista_2015_hos = indba_ptr_valor_total_2015_hos / indba_ptr_cantidad_contratistas_2015_hos, 
    indba_idr_valor_contratos_por_contratista_2016_hos = indba_ptr_valor_total_2016_hos / indba_ptr_cantidad_contratistas_2016_hos,
    indba_idr_valor_contratos_por_contratista_2017_hos = indba_ptr_valor_total_2017_hos / indba_ptr_cantidad_contratistas_2017_hos,
    indba_idr_valor_contratos_por_contratista_2018_hos = indba_ptr_valor_total_2018_hos / indba_ptr_cantidad_contratistas_2018_hos,
    indba_idr_valor_contratos_por_contratista_2019_hos = indba_ptr_valor_total_2019_hos / indba_ptr_cantidad_contratistas_2019_hos
    )
...
Diferencia entre la fehca de firma y la fecha en la que se subió el contrato

```{r}
SECOP_I %>%
  group_by(nombre_entidad, anno_firma) %>% 
  summarise(indba_ptr_dif_fechas_cargue_firma = as.numeric(mean(fecha_cargue_secop - fecha_firma_contrato))) %>%
  mutate(indicador_annio = str_c("indba_ptr_dif_fechas_cargue_firma", anno_firma, "hos", sep = "_")) %>% 
  dplyr::select(nombre_entidad, indicador_annio, indba_ptr_dif_fechas_cargue_firma) %>% 
  pivot_wider(names_from = indicador_annio, values_from = indba_ptr_dif_fechas_cargue_firma) %>% 
  dplyr::select(-indba_ptr_dif_fechas_cargue_firma_2014_hos)
...
apply(is.na(SECOP_I %>%
  ...
  dplyr::select(nombre_entidad, indicador_annio, indba_ptr_dif_fechas_cargue_firma) %>% 
  pivot_wider(names_from = indicador_annio, values_from = indba_ptr_dif_fechas_cargue_firma)%>% 
  dplyr::select(-indba_ptr_dif_fechas_cargue_firma_2014_hos)), 2, cantyprop_nas)

```

Hacer el merge de las diferencias de fechas con indicadores_bd_td

```{r}
indicadores_bd_td <- merge(indicadores_bd_td,
                           SECOP_I %>%
                             group_by(nombre_entidad, anno_firma) %>%
                             summarise(indba_ptr_dif_fechas_cargue_firma =
                                         as.numeric(mean(fecha_cargue_secop - fecha_firma_contrato))) %>%
                             mutate(indicador_annio = str_c("indba_ptr_dif_fechas_cargue_firma",
                                                            anno_firma, "hos",
                                                            sep = "_")) %>% 
                             dplyr::select(nombre_entidad, indicador_annio, indba_ptr_dif_fechas_cargue_firma) %>% 
                             pivot_wider(names_from = indicador_annio, values_from = indba_ptr_dif_fechas_cargue_firma) %>% 
...
Escribir el .csv con los Nas

```{r}
write.csv(indicadores_bd_td, file = "indicadores_bd_td_con_na.csv", row.names = TRUE)
```
...
apply(is.na(indicadores_reserva), 2, cantyprop_nas)
```

Prueba

```{r}
names(indicadores_reserva) %in% names(indicadores_bd_td)
```

Base de datos borrador

```{r}
names(indicadores_bd_td %>% dplyr::select(c(nombre_entidad, 59:ncol(indicadores_bd_td))))
```

# unir la nueva base de indicadores

```{r}
indicadores_bd_td <- merge(indicadores,
      indicadores_bd_td %>% dplyr::select(c(nombre_entidad, 59:ncol(indicadores_bd_td))),
      by = "nombre_entidad",
      all.x = TRUE,
      all.y = FALSE
      )

names(indicadores_bd_td)
```
