...
economia_depto <- economia_depto %>% 
  mutate(# Ajsutes de escritura
         ...
         indicador_annio = gsub("%", "perc_", indicador_annio),
...
indicadores_bd_td <- indicadores_bd_td %>% 
  rename("indba_riesgo_corrupcion_hos" = "ind_riesgo_corrupcion_hos")
```
...
```{r}
hospitales_finyr <- merge(SECOP_I_2 %>%
  filter(fecha_firma_contrato >= as.Date("2015-12-01") & fecha_firma_contrato <= as.Date("2015-12-31") |
         fecha_firma_contrato >= as.Date("2016-12-01") & fecha_firma_contrato <= as.Date("2016-12-31") |
         fecha_firma_contrato >= as.Date("2017-12-01") & fecha_firma_contrato <= as.Date("2017-12-31") | 
         fecha_firma_contrato >= as.Date("2018-12-01") & fecha_firma_contrato <= as.Date("2018-12-31") | 
...
indicadores_bd_td <- merge(indicadores_bd_td,
                           entidades %>% dplyr::select("indba_ptr_contratos_minima_cuantia_hos",
...
indicadores_bd_td %>% mutate(
  indba_prop_contr_finyr_total_hos = (indba_contr_finyr_hos / indba_total_cont_hos )*100 ) %>% dplyr::select(nombre_entidad, indba_prop_contr_finyr_total_hos) %>% arrange(desc(indba_prop_contr_finyr_total_hos))


# en la base de datos
indicadores_bd_td <- indicadores_bd_td %>% mutate(
  indba_prop_contr_finyr_total_hos = (indba_contr_finyr_hos / indba_total_cont_hos )*100)
```
...
```{r}
repitencia_contratista <- SECOP_I_2 %>% count(nombre_entidad, id_contratista) %>% 
