...
Lee base de datos
```{r}
SECOP_I <- read_delim(file = "C:\\Users\\Yerazo\\Documents\\Documents\\SIGMAP\\Proyecto\\SECOP_I_SFF.csv",delim = ";")
```

Leer ICFES
```{r}
ICFES11 <- read_delim(file = "C:\\Users\\Yerazo\\Documents\\SIGMAP\\Proyecto\\Saber_11__2018-2.csv",delim = ",")
```

Leer archivo id de los departamentos
```{r}
departamentos <- read_delim(file = "C:\\Users\\Yerazo\\Documents\\SIGMAP\\Proyecto\\departamentos.csv",delim = "|")
```

Leer datos de participación ciudadana y acceso, transparencia, lucha contra corrupción
```{r}
participacion<-read_delim(file = "C:\\Users\\Yerazo\\Documents\\SIGMAP\\Proyecto\\DEPTOPARTICIPACIONCIUDADANACSV.csv", delim = ";")
```

```{r}
Objeto %>% arrange(total) %>% 
  mutate(objeto_contratar=factor(objeto_contratar, levels=objeto_contratar))%>% 
           ggplot( aes(x=objeto_contratar, y=total)) +
           geom_segment( aes(xend=objeto_contratar, yend=0)) +
...
# 2.LIMPIEZA
## Renombrar las columnas y quitar registros repetidos
```{r}
SECOP_I <- distinct(SECOP_I)
...
names(SECOP_I) <- nombres
```

Se filtran los datos de los años 2014 en adelante
```{r}
SECOP_I <- SECOP_I %>% filter((anno_cargue >=2014 & anno_firma>=2014))
```

Inicialmente se tienen `r dim(SECOP_I)[1]` contratos.

```{r}
SECOP_I %>% group_by(regimen_contratacion) %>% summarise(Total = n()) 
```

```{r}
SECOP_I %>% filter(regimen_contratacion == "Estatuto General de Contratación")
```

```{r}
ent_reg <- SECOP_I %>% 
  group_by(nombre_entidad,regimen_contratacion) %>% 
  summarise(Total = n()) %>% 
  filter(regimen_contratacion == "Régimen Especial")
ent_reg
```


```{r}
ent_solo_ley80 <- all_ent %>% filter(!all_ent$nombre_entidad %in% ent_reg$nombre_entidad) %>% arrange(desc(Total))
ent_solo_ley80
```


```{r}
SECOP_I %>% filter(tipo_contrato == "Prestación de Servicios") 
```

## Sacar los NAs de plazo_ejec_calculado por no tener adjudicacion

```{r}
SECOP_I$fecha_fin_ejec <- dmy(SECOP_I$fecha_fin_ejec)
SECOP_I$fecha_ini_ejec <- dmy(SECOP_I$fecha_ini_ejec)
SECOP_I$plazo_ejec_calc <- SECOP_I$fecha_fin_ejec - SECOP_I$fecha_ini_ejec
...
Hay contratos que tienen mas de 4 años en plazo de ejecución. **¿Se deben considerar?**
...
sum(SECOP_I$valor_total)
sum(SECOP_I$valor_adiciones)
sum(SECOP_I$valor_inicial)
sum(SECOP_I$valor_estimado)
```
...
Conv_inter = SECOP_I %>%
  filter((str_detect(str_to_lower(detalle_objeto),"aunar esfuerzos") |
           str_detect(str_to_lower(detalle_objeto),"anuar esfuerzos") |
           str_detect(str_to_lower(detalle_objeto),"aunar espuerzos"))) %>% 
  dplyr::select(UID,detalle_objeto)
...
```{r}
fwrite(SECOP_I, file = "SECOP_IPS_LIMPIA.csv", sep = ";", row.names = FALSE)
```
...
Revisar el valor de contratos altos valores. Los dos primeros tienen error en sus cuantias.

```{r}
datatable(head(SECOP_I, n = 200))
```
Escribir la base de datos
...
```{r}
DT<- as.data.table(SECOP_I)
```

## EXTRAER LOS DEPARTAMENTOS Y LOS MUNICIPIOS DE CADA CONTRATO
```{r}
{# Extraer departamento----
  # Con este parte del codigo extraigo el departamento de ejecuciÃ³n
  depto_entidad <- str_sub(string = DT$municipio_ejecucion, 
               start = 1L, 
               end = str_locate(string = DT$municipio_ejecucion, pattern = "-")[,1]-2)
}


{# Extraer el municipio----
  # Con este parte del codigo extraigo el municipio de ejecuciÃ³n
  mun_entidad <- str_sub(string = DT$municipio_ejecucion, 
               start = str_locate(string = DT$municipio_ejecucion, pattern = "-")[,1]+2, 
               end = -1L)
}

{# Se agregan las nuevas columnas----
  # Con este codigo se agregan las nuevas columnas de departamento y municipio
  DT$municipio_ejecucion <- mun_entidad
  DT$departamento_ejecucion <- depto_entidad
}
```

Pasar departamentos y municipios de DT a mayúscula
```{r}
DT$departamento_ejecucion<-str_to_upper(DT$departamento_ejecucion, locale = "es")
DT$municipio_ejecucion<-str_to_upper(DT$municipio_ejecucion, locale = "es")
```

Pasar las tildes a las no tildes (áéíóú) -> (aeiou)
```{r}
DT$departamento_ejecucion<-chartr('ÁÉÍÓÚ','AEIOU',DT$departamento_ejecucion)
DT$municipio_ejecucion<-chartr('ÁÉÍÓÚ','AEIOU',DT$municipio_ejecucion)
```

## Ordenar por valor de contrato y analizar si puede existir alguno erroneo

```{r}
DT %>%  summarise(ValorContratos= sum(valor_adiciones))
DT %>%  summarise(ValorContratos= sum(valor_inicial))
DT %>%  summarise(ValorContratos= sum(valor_total))
```

```{r}
DT %>%  filter(valor_total>=1000000000000) %>%  group_by(tipo_proceso) %>%  summarise(CantidadContratos=n(),ValorContratos= sum(valor_total), ValorAdiciones = sum(valor_adiciones) )
```
```{r}
DT %>%  filter(valor_inicial >=1000000000000) %>%  group_by(tipo_proceso) %>%  summarise(CantidadContratos=n(),ValorContratos= sum(valor_inicial), ValorAdiciones = sum(valor_adiciones) )
```
```{r}
DT %>%  filter(valor_adiciones >=1000000000000) %>%  group_by(tipo_proceso) %>%  summarise(CantidadContratos=n(),ValorContratos= sum(valor_adiciones), ValorAdiciones = sum(valor_adiciones) )
```


```{r}
DT %>% group_by(estado_proceso) %>% summarise(Total=n(), Valores=sum(valor_total))
```
...
```{r}
DT  %>% filter(tipo_proceso == "Contratación Directa (Ley 1150 de 2007)") %>%  summarise(
VInicial=sum(valor_inicial), VTotal=sum(valor_total), VAdiciones=sum(valor_adiciones))
```

Cuántos contratos por CONTRATACIÓN DIRECTA superan el billón de pesos???? 
EL PIB SE COLOMBIA ES SÓLO DE 1*10^16
```{r}
datatable(DT %>%  filter(valor_total>=1000000000000) %>%  group_by(anno_firma) %>%  summarise(CantidadContratos=n(),ValorContratos= sum(valor_total), ValorAdiciones = sum(valor_adiciones) ))
```
CAUSAL CONTRATACIÓN DIRECTA
```{r}
datatable(DT %>%  filter(tipo_proceso == "Contratación Directa (Ley 1150 de 2007)" & valor_total>=1000000000000) %>%  group_by(causal_cont_direct) %>%  summarise(CantidadContratos=n(),ValorContratos= sum(valor_total), ValorAdiciones = sum(valor_adiciones)) )
```

## REEMPLAZAR LOS VALORES DEL CONTRATO
...
## GENERAL CON CONTRATACIÓN DIRECTA Y DEMÁS (nota: LO QUE NO SEA POR CONTRATACIÓN DIRECTA NO TIENE LIMPIOS LOS DATOS DE LOS VALORES DE LOS CONTRATOS)
Valor de cada año, por valor de contrato y por valor de adiciones
```{r}
DT %>% group_by(anno_firma) %>% summarise(Cantidad_Contratos=n(), ValorContratos = sum(valor_total) , ValorAdiciones=sum(valor_adiciones)) 
```
## CONTRATACIÓN DIRECTA Y DEMÁS
### Por entidades 
```{r}
ResumenGen<-DT %>%
...
ResGen_ToProce<-DT %>%
  ...
  summarise(Plazo=mean(plazo_ejec),VTotal=sum(valor_total),VAdiciones=sum(valor_adiciones),PropAdiciones=(VAdiciones/VTotal)*100)
```
...
gruposGen <- DT %>% 
  ...
  summarise(Total = n(), Valor = sum(valor_total),VAdiciones=sum(valor_adiciones),PropAdiciones=(VAdiciones/Valor)*100) %>% 
  arrange(desc(nombre_grupo))
grupos_Gen_Obj_contr <- DT %>% 
...
```{r}
DeptosG <- DT %>% group_by(departamento_ejecucion) %>% 
...
### Serie de tiempo
```{r}
#Convertir la columna de tipo caracter a tipo fecha d/m/y
DT$fecha_firma <- dmy(DT$fecha_firma)
...
#Generar gráficos
p1 <- ggplot(contratos_mes, aes(x = fecha, y = total)) + geom_line() + theme(axis.text.x = element_text(angle = 45)) + scale_x_date(date_breaks = "1 year", date_labels = "%b %y", minor_breaks = as.Date("2017-01-02")) 
  #geom_vline(xintercept = as.Date("2017-10-01"),color="red")
ggplotly(p1)
...
### Por entidades
```{r}
ResumenGDirecta<-DT %>% filter(DT$tipo_proceso == "Contratación Directa (Ley 1150 de 2007)") %>%
  group_by(nombre_entidad) %>%
  summarise(CantContratos=n(),Plazo=mean(plazo_ejec),VTotal=sum(valor_total),VAdiciones=sum(valor_adiciones),PropAdiciones=(VAdiciones/VTotal)*100, PomedioValor=mean(valor_total)) 
```
