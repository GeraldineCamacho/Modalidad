...
municipios <- merge(total_contratos,
                total_contratos_directos,
                by = "municipio_entidad",
                all.y = TRUE) %>% # porque me interesan solo los hospitales con contratos directos
  mutate(prop_directa = as.numeric(sprintf("%.2f", 
                                (contratos_directos / contratos)*100)))
...
# crear el vector que determina el orden en que quiero ver las variables
orden_mun <- c("municipio_entidad", "contratos", "contratos_directos", "prop_directa",
           "contratos_adiciones", "contratos_adiciones_dir", "prop_cont_dir_adic",
           "valor_contratos","adiciones", "adiciones_directa", "prop_dir_adic" )
...
#entidades
total_contratos <- SECOP_I %>% 
  group_by(nombre_entidad) %>% #examinar si es mejor utilizar nombre_entidad_mod
  summarise(contratos = n(),
            valor_contratos = sum(valor_total_con_adiciones),
            adiciones = sum(valor_adiciones)) 
...
# crear el vector que determina el orden en que quiero ver las variables
orden_hos <- c("nombre_entidad", "contratos", "contratos_directos", "prop_directa",
           "contratos_adiciones", "contratos_adiciones_dir", "prop_cont_dir_adic",
           "valor_contratos","adiciones", "adiciones_directa", "prop_dir_adic" )
...
# unir los detaprtamentos a los hospitales ¿CUÁL ES LA LLAVE DE UNIÓN??
entidades <- hospitales %>% 
  ## HACER SPLIT DEL NOMBRE DE LA ENTIDAD PARA SACAR EL DEPTO Y LUEGO PASAR TODO A MUNUSCULAS (EN AMBAS TABLAS PARA HACER EL JOIN)
```
