...
{# Extraer el municipio----
  ...
  #DESPUES DE ESTO HACER UN SPLIT PERO CON ";" DEJANDO SOLO EL NOMBRE DEL PRIMER MUNICIPIO
}
...
# ESTO FUNCIONA Y CORRE
```

hacer prueba de separacion de municipios

```{r}
vista_depto <- SECOP_I %>% dplyr::select(municipio_ejecucion, nombre_entidad) %>% 
  arrange(municipio_ejecucion)

mun_ejec <- arrange(mun_ejec, municipio_ejecucion)

mun_eje_entidad <- str_sub(string = mun_ejec$municipio_ejecucion, 
               start = str_locate(string = mun_ejec$municipio_ejecucion, pattern = "-")[,1]+2, 
               end = -1L)

mun_ejec$muni_calc <- mun_eje_entidad

mun_eje_entidad_new <- str_sub(string = mun_ejec$muni_calc, 
               start = str_locate(string = mun_ejec$muni_calc, pattern = ";")[,1]+2, 
               end = -1L)

mun_ejec$muni_calc_new <- mun_eje_entidad_new

#### PARA ARREGLAR ESTO HACER UN SPLIT PRIMERO HASTA EL PUNTO Y COMA DEJANDO LO DEL GUION Y LUEGO HACER OTRO SPLIT CON ESE GUION
```
...
# crear el vector que determina el orden en que quiero ver las variables
orden_mun <- c("municipio_ejecucion", "contratos", "contratos_directos", "prop_directa",
           "contratos_adiciones", "contratos_adiciones_dir", "prop_cont_dir_adic",
           "valor_contratos","adiciones", "adiciones_directa", "prop_dir_adic" )
