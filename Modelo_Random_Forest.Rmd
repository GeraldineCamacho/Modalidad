...
indicadores_clas_prep <- prep(indicadores_clas_rec)
juiced <- juice(indicadores_clas_prep)
...
El objeto indicadores_clas_rec se puede aplicar a otros objetos, pero para ello hay que prepararlo para que aprenda las instrucciones del recipe()

```{r}
indicadores_clas_train_prep <- indicadores_clas_rec %>% 
  prep() %>% 
  juice()
```
...
```{r}
set.seed(245)
rf_folds <- vfold_cv(indicadores_clas_train)
...
rf_metrics_clas <- clas_tune %>% collect_metrics()
