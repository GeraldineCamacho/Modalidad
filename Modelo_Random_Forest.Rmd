...
# Feature Selection
set.seed(111)
boruta <- Boruta(riesgo ~ ., data = indicadores_clas, doTrace = 2, maxRuns = 100)
print(boruta)
...
#get variable
not_rejected_nd_cien <- getNonRejectedFormula(boruta)
confirmed_nd_cien <- getConfirmedFormula(boruta)
```
...
```{r}
indicadores_clas_rec <- recipe(not_rejected_nd_mil, data = indicadores_clas) %>% 
...
rf_pdp_finyr_2018 <- model_profile(rf_explainer,
                              variables = "indba_ptr_contr_finyr_2018_hos",
                              N = NULL)
