...
```{r}
indicadores_clas_rec <- recipe(riesgo ~ ., data = indicadores_clas) %>% 
...
```{r}
RF_Curva_ROC_sin_boruta <- rf_val_clas %>% 
  collect_predictions() %>%
...
RF_Curva_ROC_sin_boruta
```

Guardar la imagen

```{r}
ggsave("RF_Curva_ROC_sin_boruta.png", 
       RF_Curva_ROC_sin_boruta)
```
...
```{r}
RF_importancia_sin_boruta <- importantes_clas %>% 
#  dplyr::select(-fin) %>% 
  mutate(variable = forcats::fct_reorder(variable, importancia)) %>% 
  top_n(20) %>% 
  ggplot(aes(variable, importancia)) + 
...
RF_importancia_sin_boruta
```

Guardar imagen

```{r}
ggsave("RF_importancia_sin_boruta.png",
       RF_importancia_sin_boruta)
```
...
```{r}
# Total
RF_Importancia_subcategorias_total_sin_boruta <- separate(data = importantes_clas,
         col = variable,
         into = "inicio",
         sep = "_") %>% 
  group_by(inicio) %>% 
...
# Promedio
RF_Importancia_subcategorias_media_sin_boruta <- separate(data = importantes_clas,
         col = variable,
         into = "inicio",
         sep = "_") %>% 
  group_by(inicio) %>% 
...
RF_Importancia_subcategorias_total_sin_boruta
RF_Importancia_subcategorias_media_sin_boruta  
```
...
```{r}
# Total
ggsave("RF_Importancia_subcategorias_total_sin_boruta.png",
       RF_Importancia_subcategorias_total_sin_boruta)

# Media
ggsave("RF_Importancia_subcategorias_media_sin_boruta.png",
       RF_Importancia_subcategorias_media_sin_boruta)
```
...
```{r}
# Suma total
RF_Importancia_niveles_total_sin_boruta <- importantes_clas %>%
...
# Suma media
RF_Importancia_niveles_media_sin_boruta <- importantes_clas %>%
  filter(fin == "hos" | fin == "mun" | fin == "dto") %>% 
  group_by(fin) %>% 
...
RF_Importancia_niveles_total_sin_boruta
RF_Importancia_niveles_media_sin_boruta
```
Guardar las imagenes

```{r}
# Total
ggsave("RF_Importancia_niveles_total_sin_boruta.png",
       RF_Importancia_niveles_total_sin_boruta)

# Media
ggsave("RF_Importancia_niveles_media_sin_boruta.png",
       RF_Importancia_niveles_media_sin_boruta)
```
...
RF_Importancia_años_total_sin_boruta <- importantes_clas %>% 
...
RF_Importancia_años_media_sin_boruta <- importantes_clas %>% 
  mutate(variable = gsub("_1_", "un", variable),
         variable = gsub("IC4K", "ICFK", variable),
         variable = gsub("_5_", "_cinco_", variable),
         variable = gsub("ind2", "ind_dos", variable),
...
RF_Importancia_años_total_sin_boruta
RF_Importancia_años_media_sin_boruta
```
...
```{r}
#Total
ggsave("RF_Importancia_años_total_sin_boruta.png",
       RF_Importancia_años_total_sin_boruta)

#Media
ggsave("RF_Importancia_años_media_sin_boruta.png",
       RF_Importancia_años_media_sin_boruta)
```
...
```{r}
indicadores_clas_varfin_sin_boruta <- indicadores_clas %>%
  dplyr::select(departamento_entidad,
                names(indicadores_clas_rec %>%
                        ...
                        dplyr::select(-c(departamento_entidad_Antioquia,
                                         ...
                                         orden_entidad_DISTRITO.CAPITAL,
                                         ...
                                         supers_ind10_oportunidad_entrega_reporte_info_hos_X0,
                                         supers_ind10_oportunidad_entrega_reporte_info_hos_X1
                                         )
                                      )
                      )
                ) %>% 
...
```{r}
pca_randomf_rec_sin_boruta <- recipe(riesgo ~ ., data = indicadores_clas_varfin_sin_boruta) %>%
...
pca_randomf_prep_sin_boruta <- prep(pca_randomf_rec_sin_boruta)

pca_randomf_prep_sin_boruta
```
...
```{r}
RF_PCA_hospitales_sin_boruta <- juice(pca_randomf_prep_sin_boruta) %>%
...
```{r}
#Sin leyenda
ggsave("RF_PCA_hospitales_sin_boruta_sin_leyenda.png",
       RF_PCA_hospitales_sin_boruta)

#Con leyenda
ggsave(
  "RF_PCA_hospitales_sin_boruta_con_leyenda.png",
  juice(pca_randomf_prep_sin_boruta) %>%
  ggplot(aes(PC1, PC2, label = departamento_entidad)) +
...
```{r}
umap_randomf_rec_sin_boruta <- recipe(riesgo ~ ., data = indicadores_clas_varfin_sin_boruta) %>%
  update_role(nombre_entidad, departamento_entidad, new_role = "id") %>%
  step_dummy(all_nominal(), - c(nombre_entidad, departamento_entidad), one_hot = TRUE) %>% 
  step_normalize(all_predictors()) %>%
  step_umap(all_predictors())

umap_randomf_prep_sin_boruta <- prep(umap_randomf_rec_sin_boruta)

umap_randomf_prep_sin_boruta
```
...
```{r}
RF_UMAP_hospitales_sin_boruta <- juice(umap_randomf_prep_sin_boruta) %>%
...
```{r}
#sin leyenda
ggsave("RF_UMAP_sin_boruta_sin_leyenda.png",
       RF_UMAP_hospitales_sin_boruta)

#con leyenda
ggsave(
  "RF_UMAP_sin_boruta_con_leyenda.png",
juice(umap_randomf_prep_sin_boruta) %>%
  ggplot(aes(umap_1, umap_2, label = departamento_entidad)) +
  geom_point(aes(color = departamento_entidad), alpha = 0.7, size = 2) +
  geom_text(check_overlap = TRUE, hjust = "inward", family = "IBMPlexSans") +
  labs(color = NULL,
