...
```{r}
set.seed(123)
indicadores_gen_split <- initial_split(indicadores_gen, strata = perc_contr_direct_val)
...
```{r}
indicadores_gen_rec <- recipe(perc_contr_direct_val ~ ., data = indicadores_gen) %>% 
  #update_role(nombre_entidad, new_role = "ID") %>% 
  step_normalize(all_numeric(), -perc_contr_direct_val) %>% 
...
```{r}
ggplot(test_predictions_reg, aes(perc_contr_direct_val, .pred)) +
  geom_point() + 
