...
```{r}
indicadores_clas_rec <- recipe(not_rejected_sin_mdm_supers, data = indicadores_clas) %>% 
...
```{r}
RF_Curva_ROC_sin_mdm_supers <- rf_val_clas %>% 
  collect_predictions() %>%
...
RF_Curva_ROC_sin_mdm_supers
```

Guardar la imagen

```{r}
ggsave("RF_Curva_ROC_sin_mdm_supers.png", 
       RF_Curva_ROC_sin_mdm_supers)
```
...
```{r}
RF_importancia_sin_mdm_supers <- importantes_clas %>% 
#  dplyr::select(-fin) %>% 
  mutate(variable = forcats::fct_reorder(variable, importancia)) %>% 
  top_n(20) %>% 
  ggplot(aes(variable, importancia)) + 
...
RF_importancia_sin_mdm_supers
```

Guardar imagen

```{r}
ggsave("RF_importancia_sin_mdm_supers.png",
       RF_importancia_sin_mdm_supers)
```
...
```{r}
# Total
RF_Importancia_subcategorias_total_sin_mdm_supers <- separate(data = importantes_clas,
         col = variable,
         into = "inicio",
         sep = "_") %>% 
  group_by(inicio) %>% 
...
# Promedio
RF_Importancia_subcategorias_media_sin_mdm_supers <- separate(data = importantes_clas,
         col = variable,
         into = "inicio",
         sep = "_") %>% 
  group_by(inicio) %>% 
...
RF_Importancia_subcategorias_total_sin_mdm_supers
RF_Importancia_subcategorias_media_sin_mdm_supers
```
...
```{r}
# Total
ggsave("RF_Importancia_subcategorias_total_sin_mdm_supers.png",
       RF_Importancia_subcategorias_total_sin_mdm_supers)

# Media
ggsave("RF_Importancia_subcategorias_media_sin_mdm_supers.png",
       RF_Importancia_subcategorias_media_sin_mdm_supers)
```
...
```{r}
# Suma total
RF_Importancia_niveles_total_sin_mdm_supers <- importantes_clas %>%
...
# Suma media
RF_Importancia_niveles_media_sin_mdm_supers <- importantes_clas %>%
  filter(fin == "hos" | fin == "mun" | fin == "dto") %>% 
  group_by(fin) %>% 
...
RF_Importancia_niveles_total_sin_mdm_supers
RF_Importancia_niveles_media_sin_mdm_supers
```
Guardar las imagenes

```{r}
# Total
ggsave("RF_Importancia_niveles_total_sin_mdm_supers.png",
       RF_Importancia_niveles_total_sin_mdm_supers)

# Media
ggsave("RF_Importancia_niveles_media_sin_mdm_supers.png",
       RF_Importancia_niveles_media_sin_mdm_supers)
```
...
RF_Importancia_años_total_sin_mdm_supers <- importantes_clas %>% 
...
RF_Importancia_años_media_sin_mdm_supers <- importantes_clas %>% 
  mutate(variable = gsub("_1_", "un", variable),
         variable = gsub("IC4K", "ICFK", variable),
         variable = gsub("_5_", "_cinco_", variable),
         variable = gsub("ind2", "ind_dos", variable),
...
RF_Importancia_años_total_sin_mdm_supers
RF_Importancia_años_media_sin_mdm_supers
```
...
```{r}
#Total
ggsave("RF_Importancia_años_total_sin_mdm_supers.png",
       RF_Importancia_años_total_sin_mdm_supers)

#Media
ggsave("RF_Importancia_años_media_sin_mdm_supers.png",
       RF_Importancia_años_media_sin_mdm_supers)
```
...
```{r}
indicadores_clas_varfin_sin_mdm_supers <- indicadores_clas %>%
...
```{r}
pca_randomf_rec_sin_mdm_supers <- recipe(riesgo ~ ., data = indicadores_clas_varfin_sin_mdm_supers) %>%
...
pca_randomf_prep_sin_mdm_supers <- prep(pca_randomf_rec_sin_mdm_supers)

pca_randomf_prep_sin_mdm_supers
```
...
```{r}
RF_PCA_hospitales_sin_mdm_supers <- juice(pca_randomf_prep_sin_mdm_supers) %>%
...
RF_PCA_hospitales_sin_mdm_supers
```
...
```{r}
#Sin leyenda
ggsave("RF_PCA_hospitales_sin_mdm_supers_sin_leyenda.png",
       RF_PCA_hospitales_sin_mdm_supers)

#Con leyenda
ggsave(
  "RF_PCA_hospitales_sin_mdm_supers_con_leyenda.png",
  juice(pca_randomf_prep_sin_mdm_supers) %>%
  ggplot(aes(PC1, PC2, label = departamento_entidad)) +
...
```{r}
umap_randomf_rec_sin_mdm_supers <- recipe(riesgo ~ ., data = indicadores_clas_varfin_sin_mdm_supers) %>%
  update_role(nombre_entidad, departamento_entidad, new_role = "id") %>%
  step_dummy(all_nominal(), - c(nombre_entidad, departamento_entidad), one_hot = TRUE) %>% 
  step_normalize(all_predictors()) %>%
  step_umap(all_predictors())

umap_randomf_prep_sin_mdm_supers <- prep(umap_randomf_rec_sin_mdm_supers)

umap_randomf_prep_sin_mdm_supers
```
...
```{r}
RF_UMAP_hospitales_sin_mdm_supers <- juice(umap_randomf_prep_sin_mdm_supers) %>%
...
RF_UMAP_hospitales_sin_mdm_supers
```
...
```{r}
#sin leyenda
ggsave("RF_UMAP_sin_mdm_supers_sin_leyenda.png",
       RF_UMAP_hospitales_sin_mdm_supers)

#con leyenda
ggsave(
  "RF_UMAP_sin_mdm_supers_con_leyenda.png",
juice(umap_randomf_prep_sin_mdm_supers) %>%
  ggplot(aes(umap_1, umap_2, label = departamento_entidad)) +
  geom_point(aes(color = departamento_entidad), alpha = 0.7, size = 2) +
  geom_text(check_overlap = TRUE, hjust = "inward", family = "IBMPlexSans") +
  labs(color = NULL,
