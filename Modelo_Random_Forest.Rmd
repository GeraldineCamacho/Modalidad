...
indicadores_clas$ind_riesgo_corrupcion <- NULL

#crear los rangos
indicadores_clas <- indicadores_clas %>% 
  mutate(riesgo = case_when(perc_contr_directa_num <= 25 ~ "Bajo o Moderado",
                            perc_contr_directa_num <= 55 ~ "Medio",
                            TRUE ~ "Alto"))
...
separate(data = importantes_clas,
  ...
  ggplot() +
    ...
    coord_flip() +
    labs(x = "Importancia",
         y = "Subcategoría de indicador",
         title = "Importancia de las subcategorías")+
    theme(plot.title = element_text(hjust = 0.5))
...
# Para que coincidan los nombres
...
```{r}
prueba_union <- indicadores_clas[ , c(1:18, 182)]
```

```{r}
supersalud %>% group_by(NIT)
supersalud %>% distinct(Entidad)
prueba_union %>% distinct(nit_entidad)
prueba_union %>% distinct(nombre_entidad)
...
prueba_union <- prueba_union %>% 
  separate(nombre_entidad, sep = " - ", into = c("Depto", "Hospital")) %>% 
  dplyr::select(-Depto) %>% 
  separate(nit_entidad, sep = "-", into = "nit_entidad")
  
#eliminar tildes
prueba_union$Hospital <- chartr('ÁÉÍÓÚ', 'AEIOU', prueba_union$Hospital)

#eliminar puntos
prueba_union$Hospital <- gsub("[.]", "", prueba_union$Hospital)

prueba_union$nit_entidad <- gsub("[.]", "", prueba_union$nit_entidad)
  
#ver cuántos coinciden 
supersalud %>% filter(Entidad %in% prueba_union$Hospital)
...
# Los que sí coinciden por nit o por nombre (787)
prueba_union %>% filter(!(nit_entidad %in% supersalud$NIT) &
                                            (Hospital %in% supersalud$Entidad))

coinciden_prueba <- prueba_union %>% 
  filter(!(nit_entidad %in% supersalud$NIT) &
        (Hospital %in% supersalud$Entidad)) 
#%>% 
 
supersalud %>% filter(Entidad %in% coinciden_prueba$Hospital)

  merge(x = coinciden_prueba, y = supersalud,
        by.x = "Hospital",
        by.y = "Entidad",
        all.x = TRUE,
        all.y = FALSE) %>% 
  select(-nit_entidad) %>% 
  rename("nit_entidad" = "NIT") %>% 
  select(names(prueba_union))
