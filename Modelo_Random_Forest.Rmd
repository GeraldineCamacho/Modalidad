...
```{r}
RF_Curva_ROC_indba <- rf_val_clas %>% 
  collect_predictions() %>%
...
```{r}
RF_importancia_indba <- importantes_clas %>% 
#  dplyr::select(-fin) %>% 
  mutate(variable = forcats::fct_reorder(variable, importancia)) %>% 
  top_n(20) %>% 
  ggplot(aes(variable, importancia)) + 
...
RF_importancia_indba
```

Guardar imagen

```{r}
ggsave("RF_importancia_indba.png",
       RF_importancia_indba)
```
...
```{r}
# Total
RF_Importancia_subcategorias_total_indba <- separate(data = importantes_clas,
         col = variable,
         into = "inicio",
         sep = "_") %>% 
  group_by(inicio) %>% 
...
# Promedio
RF_Importancia_subcategorias_media_indba <- separate(data = importantes_clas,
         col = variable,
         into = "inicio",
         sep = "_") %>% 
  group_by(inicio) %>% 
...
RF_Importancia_subcategorias_total_indba
RF_Importancia_subcategorias_media_indba
```
...
```{r}
# Total
ggsave("RF_Importancia_subcategorias_total_indba.png",
       RF_Importancia_subcategorias_total_indba)

# Media
ggsave("RF_Importancia_subcategorias_media_indba.png",
       RF_Importancia_subcategorias_media_indba)
```
