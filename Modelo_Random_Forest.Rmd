...
Prueba con solo indba

```{r}
indicadores_clas <- indicadores_clas %>% 
  pivot_longer(cols = !c(nombre_entidad, orden_entidad, departamento_entidad),
               names_to = "indicador",
               values_to = "valor") %>% 
  filter(str_detect(indicador, pattern = "indba")) %>% 
...
## Eliminar algunas variables para volver a correr el boruta

```{r}
#supersalud
indicadores_clas$supers_ind10_oportunidad_entrega_reporte_info_hos <- NULL
indicadores_clas$supers_ind2_efectividad_en_auditoria_hos <- NULL
#mdm
indicadores_clas$inmdm_clasificacion_mun <- NULL
indicadores_clas$inmdm_gestion_ejec_recursos_mun <- NULL
indicadores_clas$inmdm_gestion_mov_recursos_mun <- NULL 
indicadores_clas$inmdm_gestion_ord_territorial_mun <- NULL
indicadores_clas$inmdm_gobno_abierto_transparencia_mun <- NULL
indicadores_clas$inmdm_ind_ppuntaje_res_2018_mun <- NULL
indicadores_clas$inmdm_puntaje_gestion_mun <- NULL
indicadores_clas$inmdm_puntaje_mdm_2018_mun <- NULL
```
...
#get variable
not_rejected_nd <- getNonRejectedFormula(boruta)
confirmed_nd <- getConfirmedFormula(boruta)
```
...
```{r}
indicadores_clas_varfin <- indicadores_clas %>% 
...
```{r}
juice(pca_randomf_prep) %>%
  ggplot(aes(PC1, PC2, label = nombre_entidad)) +
...
```{r}
juice(umap_randomf_prep) %>%
  ggplot(aes(umap_1, umap_2, label = nombre_entidad)) +
  geom_point(aes(color = departamento_entidad), alpha = 0.7, size = 2) +
  geom_text(check_overlap = TRUE, hjust = "inward", family = "IBMPlexSans") +
  labs(color = NULL)
```
