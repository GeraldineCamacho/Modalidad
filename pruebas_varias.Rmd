...
```{r}
sapply(financiero_municipio, function(x) sum(is.na(x)))

financiero_municipio %>% na.omit() %>% count(codigo_entidad) %>% arrange(desc(n))
```

```{r}
financiero_municipio %>% dplyr::select(codigo_entidad, Indicador.de.desempeño.fiscal ) %>% na.omit() %>% count(codigo_entidad) %>% arrange(desc(n))

financiero_municipio %>% na.omit(Indicador.de.desempeño.fiscal) %>% count(codigo_entidad) %>% arrange(desc(n))

financiero_municipio %>% na.omit(financiero_municipio$Número.total.de.proyectos) %>% count(codigo_entidad) %>% arrange(desc(n))

financiero_municipio %>% na.omit(financiero_municipio$Valor.del.número.total.de.proyectos) %>% count(codigo_entidad) %>% arrange(desc(n))
```
