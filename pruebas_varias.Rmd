...
#depto
orden_depto<- paste(orden_depto, "depto", sep = "_")
names(deptos) <- orden_depto

#municipios
orden_mun <- paste(orden_mun, "mun", sep = "_")
names(municipios) <- orden_mun

#hospitales
orden_hos <- paste(orden_hos, "hos", sep = "_")
...
indicadores_bd_td <- merge( indicadores_bd_td,
                            entidades %>% dplyr::select( nombre_entidad_hos,
                                                         contratos_minima_cuantia_hos,
                                                         prop_contr_minima_cuantia_hos,
                                                         valor_contratos_minima_cuantia_hos,
                                                         prop_val_minima_cuantia_hos,
                                                         contratos_minima_cuantia_depto,
                                                         prop_contr_minima_cuantia_depto,
                                                         valor_contratos_minima_cuantia_depto,
                                                         prop_val_minima_cuantia_depto),
                            by.x = "nombre_entidad",
                            by.y = "nombre_entidad_hos",
                            all.x = TRUE )
```
...
Añadir inmdm_clasificacion porque se daño cuando se pasó a numérico

```{r}
#indicadores_bd_td$inmdm_clasificacion <- NULL

indicadores_bd_td <- merge(indicadores_bd_td,
      MDM %>% dplyr::select(cod_entidad, inmdm_clasificacion),
      by = "cod_entidad",
      all.x = TRUE,
      all.y = FALSE)
...
```{r}
write.csv(indicadores_bd_td, file = "indicadores_bd_td_v2.csv", row.names = TRUE)
```
...
indicadores_bd_td %>% mutate(
  indba_prop_contr_finyr_dto_total_hos = (indba_contr_finyr_hos / indba_total_contratos )*100) %>% dplyr::select(nombre_entidad, indba_prop_contr_finyr_dto_total_hos) %>% arrange(desc(indba_prop_contr_finyr_dto_total_hos))


# en la base de datos
indicadores_bd_td <- indicadores_bd_td %>% mutate(
  indba_prop_contr_finyr_dto_total_hos = (indba_contr_finyr_hos / indba_total_contratos )*100)
```
